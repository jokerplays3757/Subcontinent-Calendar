...
const VS_2026_MONTH_STARTS: { monthIndex: number; start: Date }[] = [
  { monthIndex: 0, start: new Date(2026, 2, 19) },
  { monthIndex: 1, start: new Date(2026, 3, 17) },
  { monthIndex: 2, start: new Date(2026, 4, 17) },
  { monthIndex: 3, start: new Date(2026, 5, 16) },
  { monthIndex: 4, start: new Date(2026, 6, 16) },
  { monthIndex: 5, start: new Date(2026, 7, 15) },
  { monthIndex: 6, start: new Date(2026, 8, 14) },
  { monthIndex: 7, start: new Date(2026, 9, 14) },
  { monthIndex: 8, start: new Date(2026, 10, 13) },
  { monthIndex: 9, start: new Date(2026, 11, 13) },
];

const SAKA_2026_MONTH_STARTS: { monthIndex: number; start: Date }[] = [
  { monthIndex: 0, start: new Date(2026, 2, 19) },
  { monthIndex: 1, start: new Date(2026, 3, 19) },
  { monthIndex: 2, start: new Date(2026, 4, 19) },
  { monthIndex: 3, start: new Date(2026, 5, 18) },
  { monthIndex: 4, start: new Date(2026, 6, 18) },
  { monthIndex: 5, start: new Date(2026, 7, 17) },
  { monthIndex: 6, start: new Date(2026, 8, 17) },
  { monthIndex: 7, start: new Date(2026, 9, 18) },
  { monthIndex: 8, start: new Date(2026, 10, 17) },
  { monthIndex: 9, start: new Date(2026, 11, 17) },
];
...
export function gregorianToVikramSamvat(
  ...
  if (gYear === 2026) {
    ...
    if (date < chaitraStart) {
      vsYear = 2082;
      if (gMonth === 0) { monthIndex = gDay < 14 ? 9 : 10; currentStart = new Date(2026, 0, gDay < 14 ? 1 : 14); }
      else if (gMonth === 1) { monthIndex = gDay < 12 ? 10 : 11; currentStart = new Date(2026, 1, gDay < 12 ? 1 : 12); }
      else { monthIndex = 11; currentStart = new Date(2026, 2, 1); }
    } else {
      vsYear = 2083;
      monthIndex = 0;
    ...
    if (gMonth < 2 || (gMonth === 2 && gDay < 14)) {
      ...
      currentStart = new Date(gYear, gMonth, 1); // Simple approx
    } else {
      vsYear = gYear + 57;
      monthIndex = gMonth === 2 ? 0 : 1; // Simplified approx
      currentStart = new Date(gYear, 2, 19);
    }
  }

  const day = Math.floor((date.getTime() - currentStart.getTime()) / (1000 * 60 * 60 * 24)) + 1;
...
export function gregorianToSaka(date: Date): { year: number; month: string; monthIndex: number; day: number } {
  ...
  if (gYear === 2026) {
    ...
    monthIndex = 0; // Simplified approx
...
// Zodiac logic stays as in your file
export function getVedicZodiac(date: Date) {
  const month = date.getMonth();
  const day = date.getDate();
  const boundaries = [[0, 14], [1, 13], [2, 15], [3, 14], [4, 15], [5, 15], [6, 17], [7, 17], [8, 17], [9, 17], [10, 16], [11, 16]];
